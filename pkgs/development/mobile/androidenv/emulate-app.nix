...
stdenv.mkDerivation {
  ...
  buildCommand = ''
    ...
    export PATH="${androidsdkComposition}/bin:$PATH"
    ...
    do
        if [ -z "$(adb devices | grep emulator-$i)" ]
        then
            port=$i
            break
        fi
    ...
    then
        ...
        yes "" | android create avd -n device -t ${if useGoogleAPIs then "'Google Inc.:Google APIs:"+platformVersion+"'" else "android-"+platformVersion} $NIX_ANDROID_AVD_FLAGS
    ...
    # Launch the emulator
    emulator -avd device -no-boot-anim -port $port $NIX_ANDROID_EMULATOR_FLAGS &
    ...
    while [ -z "$(adb -s emulator-$port shell getprop dev.bootcomplete | grep 1)" ]
    do
        sleep 5
    done
    ...
    #while [ -z "$(adb -s emulator-$port shell getprop sys.boot_completed | grep 1)" ]
    #do
        #sleep 5
    #done
    ...
    ${stdenv.lib.optionalString (app != null) ''
      ...
      then
          ...
          adb -s emulator-$port install "$appPath"
      fi

      # Start the application
      ${stdenv.lib.optionalString (package != null && activity != null) ''
          adb -s emulator-$port shell am start -a android.intent.action.MAIN -n ${package}/${activity}
          APPPID=`adb -s emulator-$port -d shell ps | grep "${package}" | cut -c10-15 | sed -e 's/ //g'`
          adb -s emulator-$port -d logcat -v long \
            | tr -d '\r' | sed -e '/^\[.*\]/ {N; s/\n/ /}' | grep -v '^''$' \
            | grep " $APPPID:"
      ''}
    ''}
